...
void console_display(){   //-------------------------> printing important data to the brain
  while (true) {
    ...
    std::string autoStr = selector.get_auton()->name;
    ...
    console.printf("[ArmDeg %.4f]", float(ArmSensor.get_position()) / 100);
    pros::delay(ez::util::DELAY_TIME);
    console.clear(); //------------------------------------------------------> Refreshes screen after delay to save resources
  }
}
pros::Task BrainScreenTask(console_display);
...
pros::Task ControllerScreenTask(controller_screen);
...
void ArmPID(){
  ...
  while(true){
    float output = armPID.compute(ArmSensor.get_position() / 100.0);
    ArmL.move(output);
    ArmR.move(output);
    pros::delay(ez::util::DELAY_TIME);
  }
}
pros::Task ArmPIDTask(ArmPID);
...
void initialize() {
  ...
  ArmTarget currentTarget = ArmTarget::HOME;//------------> Sets arm starting pos to be down
  armPID.exit_condition_set(
    ...
    500,
    500);
  ArmSensor.reset();             //-----------------------> Resets the rotation sensor for the arm
  ArmSensor.set_position(0);     //-----------------------> Sets the rotation sensor's position value to 0
...
void opcontrol() {
  ...
  ArmTarget currentTarget = ArmTarget::HOME;

  pros::Task DriverArmMacro([&]() {
    while(true)
    {
      // Move to next target
      if (master.get_digital_new_press(DIGITAL_DOWN)
       && !master.get_digital(DIGITAL_RIGHT)) {
        switch (currentTarget) {
          case ArmTarget::HOME:         
            currentTarget = ArmTarget::GRAB_RING; break;
          case ArmTarget::GRAB_RING:    
            currentTarget = ArmTarget::SCORE_RING; break;
          case ArmTarget::SCORE_RING:   
            currentTarget = ArmTarget::TIPPING_GOAL; break;
          default: 
            break;
        }
      }
      ...
      if (master.get_digital_new_press(DIGITAL_RIGHT)
       && !master.get_digital(DIGITAL_DOWN)) {
        switch (currentTarget) {
          case ArmTarget::TIPPING_GOAL: 
            currentTarget = ArmTarget::SCORE_RING; break;
          case ArmTarget::SCORE_RING:   
            currentTarget = ArmTarget::RETURN_GRAB; break;
          case ArmTarget::RETURN_GRAB:  
            currentTarget = ArmTarget::HOME; break;
          default: 
            break;
        }
      }
      ...
      // Set PID target 
      float targetPosition = getArmTargetPosition(currentTarget);
      armPID.target_set(targetPosition);
...
//---------------------------------------------------------Intake code------------------------------------------------------------------
    ...
    if/*---*/(master.get_digital(DIGITAL_R1) == true ) {
      ...
      &&  !isSorting 
      && currentTarget != ArmTarget::RETURN_GRAB
      && currentTarget != ArmTarget::GRAB_RING) {
      Intake.move_velocity(600);
    } else if (!isSorting) {
